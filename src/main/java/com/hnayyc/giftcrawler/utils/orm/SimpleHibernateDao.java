...
import com.hnayyc.giftcrawler.utils.ReflectionUtils;
...
//	}
    ...
    public Connection getConnection() throws SQLException {
        if (sessionFactory instanceof SessionFactoryImplementor) {
            // TODO hibernate-4.3.11已经弃用这种方式，找到推荐方式。
    ...
    public void save(final List<T> entity) throws HibernateException{
        Assert.notNull(entity, "entity不能为空");
    ...
    public Query createHQLQuery(final String queryString,String sa,final Map<String, ?> values) {
        Assert.hasText(queryString, "queryString不能为空");
        Query query = getSession().createQuery(queryString);
        if (values != null) {
            query.setProperties(values);
     ...
     * @return
     */
    public List createSQLQueryForEntity(final String sqlQueryString, final Object... values) {
        Assert.hasText(sqlQueryString, "queryString不能为空");
        SQLQuery query = getSession().createSQLQuery(sqlQueryString);
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
        ...
        List list = query.addEntity(entityClass).list();
        return list;
    }
}
